...
// OffsetScrollAdapterDirective can be used for advanced cases where scroll offset
// calculations need to account for header height. Not needed with explicit wrapper height.
// import { OffsetScrollAdapterDirective } from './offset-scroll-adapter.directive';
  ...
  template: `
    ...
    <ion-content [scrollY]="false">
      ...
      <div #scrollContainer class="scroll-container ion-content-scroll-host" vdndScrollable>
        ...
        <!-- Virtual list with explicit height for absolute positioning -->
        <div
        ...
        >
          ...
          >
            @if (isPlaceholder) {
              ...
              <div class="task-item" vdndDraggable [attr.data-draggable-id]="task.id">
...
export class PageScrollDemoComponent implements OnDestroy {
  ...
  // Expose scroll container element for the offset adapter
  readonly scrollContainerEl = computed(() => this.scrollContainer().nativeElement);
    ...
    afterNextRender(() => {
      ...
      this.#resizeObserver = new ResizeObserver((entries) => {
        for (const entry of entries) {
          this.headerHeight.set(entry.target.getBoundingClientRect().height);
        }
      });
